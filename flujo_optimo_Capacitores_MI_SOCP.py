...
for i in range(1,n):
    res += [cvx.abs(sk[i]) <= 0]                                               # solo un nodo de holgura

for i in range(l):
    admitancia=1/(rx[i,0]+rx[i,1]*1j)
    res+=[Skm[i]==admitancia.conjugate()*(u[int(nodos_l[i,0])]-wl[i])]
    res+=[Smk[i]==admitancia.conjugate()*(u[int(nodos_l[i,1])]-cvx.conj(wl[i]))]
...
#
# z = cvx.Variable((n,len(capacitores)), boolean=True) #variable binaria
...
#
#
#     res+=[sk[k]-pq_activa[]+(skc[k]*1j)==(Apositiva[k,:]@Sls)+(Anegativa[k,:]@Slr)]
#     res+=[u[k]>=0.8]
#     res+=[u[k]<=1.1]
#     res+=[cvx.abs(sk[0])<=100]
#     res+=[cvx.real(sk[0])>=0]
#     Funcion_objetivo = Funcion_objetivo + sk[k] - demanda +(skc[k]*1j)
#
# for i in range(1,n):
#     res += [cvx.abs(sk[i]) <= 0]
#
...
#
# for i in range(l):
#     admitacia=1/(datos[i,2]+datos[i,3]*1j)
#     admitanciaconjugada=admitacia.conjugate()
#     res+=[Sls[i]==admitanciaconjugada*(u[int(datos[i,0])]-wl[i])]
#     res+=[Slr[i]==admitanciaconjugada*(u[int(datos[i,1])]-cvx.conj(wl[i]))]
#     res+=[cvx.SOC(u[int(datos[i,0])]+u[int(datos[i,1])],cvx.vstack([2 * wl[i],u[int(datos[i,0])]-u[int(datos[i,1])]]))]
#
#
# obj = cvx.Minimize(cvx.real(Funcion_objetivo))
# Z= cvx.Problem(obj,res)
# Z.solve("MOSEK")
# print("Valor de las perdidas", obj.value*potencia_base, Z.status)
#
# print("Tensiones en los nodos")
# print((np.sqrt(u.value)))
#
# print("Potencia generada en el Nodo Slack")
# print(cvx.real(sk.value[0]))
#
# #graficas
# ancho=10
# alto=5
...
# plt.figure(2,figsize=(ancho, alto))
# plt.plot(x,abs(datos[:,4]+1j*datos[:,5]),"purple")
# plt.plot(x,abs(datos[:,4]+1j*datos[:,5]),"o",mfc="black",mec="purple")
# plt.xlabel('Nodo', fontsize=13)    #definir el eje x
# plt.ylabel('Demanda', fontsize=13)  # definir el eje y
# plt.xlim(0,34)
# plt.grid()
# plt.show()